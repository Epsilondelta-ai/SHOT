...
export async function getSerializedRoomPlayers(roomId: string): Promise<SerializedRoomPlayer[]> {
	...
	const humanIds = players.filter((player) => player.playerType === 'human').map((player) => player.userId);
